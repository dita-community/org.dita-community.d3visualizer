...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template mode="generate-index-tree-graph" match="*[df:class(., 'map/map')]">
    ...
    <xsl:text>label=</xsl:text><xsl:sequence select="d3:quoteString(string-join(('Map index tree: ', $mapTitle), ''))"/>
    <xsl:text>;&#x0a;</xsl:text>
    ...
    <xsl:text> [</xsl:text>
    <xsl:sequence select="d3:makeProperty('label', 'Index')"/>
    <xsl:text>]
    </xsl:text>
...
// End of index hierarchy 
    ...
    <xsl:sequence select="d3:makeProperties(
      ('shape', 'record', 
      'color', 'blue',
      'style', 'filled',
      'fillcolor', 'yellow'))"/>
  ...
  <xsl:template mode="link-one-level" match="index-terms:index-group | index-terms:index-term">
    <xsl:sequence select="d3:quoteString(string(index-terms:label))"/>
    <xsl:text>&#x0a;</xsl:text>
  </xsl:template>
  ...
  <xsl:template mode="generate-edges" match="index-terms:index-group | index-terms:index-term">
    ...
    <xsl:sequence select="d3:quoteString($myNodeId)"/>
    <xsl:text> -> {      
    </xsl:text>
