...
<xsl:stylesheet
  ...
  xmlns:d3="http://dita4publishers.sourceforge.net/functions/d3"

  exclude-result-prefixes="xs xd df relpath html2 d3 enum index-terms"
  version="2.0">
  
  <!-- =============================================================
    
       DITA Map to D3 Visualization 
       
       This plugin provides transforms that generate data sets and
       HTML files for use with the D3 visualization Javascript
       library. See d3js.org.
  ...
  <xsl:import href="plugin:org.dita-community.common.mapdriven:xsl/indexProcessing.xsl"/>
  
  <xsl:import href="plugin:org.dita.xhtml:xsl/dita2xhtml.xsl"/>
  
  <xsl:param name="outdir" select="./d3"/>
  ...
  <xsl:variable name="generateIndexBoolean" 
    select="matches($generateIndex, '1|yes|on|true', 'i')"/>
  ...
  <xsl:variable name="generateGlossaryBoolean" 
    select="matches($generateGlossary, '1|yes|on|true', 'i')"
  />
  
  
  <!-- We're generating d3 JSON files, which are plain text files -->
  ...
  <xsl:include href="map2d3Common.xsl"/>
  <xsl:include href="map2d3Functions.xsl"/>
